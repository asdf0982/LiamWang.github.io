...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        self.params['b1'] = np.zeros((1,hidden_dim)) #1,H
        self.params['W2'] = weight_scale * np.random.randn(hidden_dim,num_classes) #H,C
        self.params['b2'] = np.zeros((1,num_classes)) #1,C
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in xrange(1, self.num_layers):
        	layer[i], cache_layer[i] = affine_relu_forward(layer[i-1],
        												   self.params['W%d' % i],
        												   self.params['b%d' % i])
        WL = 'W%d' % self.num_layers
        bL = 'b%d' % self.num_layers
        ...
        # add regularzation loss:
        for i in xrange(1, self.num_layers + 1):
        	loss += 0.5 * self.reg * np.sum(self.params['W%d' % i]**2)
        # Backprop into last layer
        dx = {}
        dx[self.num_layers], grads[WL], grads[bL] = affine_backward(dscores,cache_scores)
        grads[WL] += self.reg * self.params[WL]
        # Backprop into last layer
